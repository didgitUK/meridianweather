...
**Further reading:** [CHANGELOG.md](CHANGELOG.md) · [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) · [docs/DECISIONS.md](docs/DECISIONS.md) · [docs/SECURITY.md](docs/SECURITY.md) · [docs/OBSERVABILITY.md](docs/OBSERVABILITY.md) · [docs/ADSENSE-CONTENT-REVIEW.md](docs/ADSENSE-CONTENT-REVIEW.md)
...
- **Header search** — find a place immediately; footer links **About**, **FAQ**, Journal, Legal, Docs.
...
- **7 UI locales** (`en`, `en-GB`, `de`, `fr`, `es`, `ja`, `ar`); US English (`en`) defaults to **°F**, others to **°C**. Search/AdSense sitemap is **`en` / `en-GB` only** after the low-value content remediation.
...
| `npm run populate:place-content` | Generate place POIs + guide **drafts** for hot UK places (admin publish required; stub fails publish validation) |
| `npm run unpublish:stub-guides` | Unpublish stub/filler place guides (AdSense remediation) |
| `npm run unpublish:all-guides` | Unpublish every published place guide |
...
- **`/about`**, **`/faq`**, UK `/weather/{slug}` place pages with Things to do (OSM), optional human-published guides, hero weather theater
...
- AdSense site approval depends on Google’s review after content remediation — follow [`docs/ADSENSE-CONTENT-REVIEW.md`](docs/ADSENSE-CONTENT-REVIEW.md). Do not mass-publish stub/LLM guides before approval.
...
- Gandi Simple Hosting **Starter** allows one vhost (apex only). `www` HTTPS needs a plan upgrade or a proper web-forward — see OPS-RUNBOOK.
- `/manifest.webmanifest` may 404 when middleware locale-prefixes the path; PWA install still works from supported browsers where the asset is reachable.
...
| [docs/ADSENSE-CONTENT-REVIEW.md](docs/ADSENSE-CONTENT-REVIEW.md) | AdSense low-value fix + re-review checklist |
| [docs/BUILD-TIMELINE.md](docs/BUILD-TIMELINE.md) | Honest phase timeline |
...
| [docs/PWA.md](docs/PWA.md) | Install / offline / push |
